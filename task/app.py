...
# ============================================================
app = Flask(__name__)
# SECRET_KEY: MUST be set in production (Vercel env var)
_secret = os.environ.get('SECRET_KEY', '')
if not _secret:
    _secret = os.urandom(32).hex()  # Random key per-deployment (safe for stateless Vercel)
    import warnings; warnings.warn('SECRET_KEY not set — using random key (sessions will not persist across cold starts)')
...
def set_security_headers(response):
    ...
    # CSP: allow only necessary resources
    ...
    # Set rate limit cookies if any
    if hasattr(request, '_rate_limit_cookies'):
        for key, value in request._rate_limit_cookies.items():
            response.set_cookie(key, value, max_age=LOGIN_WINDOW, httponly=True, samesite='Lax')
    return response
...
def not_found(e):
    ...
    return render_template('login.html', lang=session.get('lang', 'id')), 404

@app.errorhandler(500)
def server_error(e):
    """Generic 500 error — no info disclosure"""
    if request.path.startswith('/api/'):
...
# ============================================================
# RATE LIMITING (cookie-based for Vercel serverless)
# ============================================================
...
def _get_rate_limit_key():
    """Generate a rate limit key from IP + User-Agent"""
    ip = request.remote_addr or 'unknown'
    ua = request.headers.get('User-Agent', '')[:50]
    return _hashlib.md5(f'{ip}:{ua}'.encode()).hexdigest()[:12]

def is_rate_limited(ip):
    """Check if request has exceeded login attempts (cookie-based)"""
    key = f'_rl_{_get_rate_limit_key()}'
    data = request.cookies.get(key, '')
    if not data:
        return False
    try:
        timestamps = [float(t) for t in data.split(',')]
        now = _time.time()
        # Remove old attempts
        timestamps = [t for t in timestamps if now - t < LOGIN_WINDOW]
        return len(timestamps) >= MAX_LOGIN_ATTEMPTS
    except:
        return False

def record_login_attempt(ip):
    """Record a login attempt in cookie"""
    ...
    # Remove old attempts
    timestamps = [t for t in timestamps if now - t < LOGIN_WINDOW]
    # Store as cookie (response will be set later)
    if not hasattr(request, '_rate_limit_cookies'):
        request._rate_limit_cookies = {}
    request._rate_limit_cookies[key] = ','.join(str(t) for t in timestamps[-10:])  # Keep last 10
...
if not ADMIN_PASSWORD:
    import warnings; warnings.warn('ADMIN_PASSWORD not set — login disabled')
...
def login_page():
    ...
    if request.method == 'POST':
        ...
        if is_rate_limited(client_ip):
            flash('Terlalu banyak percobaan! Coba lagi dalam 5 menit.', 'error')
            return render_template('login.html', lang=session.get('lang', 'id'))
        
        password = request.form.get('password', '')
        username = request.form.get('username', '')
        if not ADMIN_PASSWORD:
            flash('Login tidak dikonfigurasi!', 'error')
...
def set_language(lang_code):
    ...
    # OPEN REDIRECT FIX: Only allow relative paths starting with /
    next_url = request.args.get('next', request.referrer or url_for('index'))
    # Validate: must be relative path, no protocol, no protocol-relative
    from urllib.parse import urlparse
    parsed = urlparse(next_url)
    # Block if: has scheme (http/https/javascript/data), starts with //, or has netloc
    if parsed.scheme or parsed.netloc or next_url.startswith('//'):
        next_url = url_for('index')
    # Also ensure it starts with / (relative path)
    elif not next_url.startswith('/'):
        next_url = url_for('index')
    return redirect(next_url)

# Database: use /tmp on Vercel (ephemeral but writable), local file otherwise
# NOTE: SQLite in /tmp is ephemeral on Vercel — data resets on cold start
# For production, use PostgreSQL or Firestore (already configured)
...
def push_to_github():
    """Smart push to GitHub via browser — only uploads changed files"""
    ...
    if request.method == 'GET':
        push_html = '''<!DOCTYPE html>
<html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Push to GitHub</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:system-ui;background:#0f0f0f;color:#e0e0e0;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
.card{max-width:500px;width:100%;background:#1a1a2e;border-radius:16px;padding:36px}
...
.btn{width:100%;padding:14px;background:#4CAF50;border:none;border-radius:8px;color:#fff;font-size:15px;font-weight:700;cursor:pointer}
.btn:hover{background:#45a049}
.btn:disabled{background:#333}
#status{margin-top:16px;padding:14px;border-radius:8px;display:none;font-size:13px;line-height:1.7}
...
<body>
<div class="card">
  <h1>&#x1F4E6; Smart Push to GitHub</h1>
  <p class="sub">Only pushes changed files (fast!)</p>
  <form id="pushForm">
    <input type="password" id="tok" placeholder="ghp_xx...xxxx" required autocomplete="off">
    <button class="btn" type="submit" id="btn">&#x1F680; Push Changes</button>
  </form>
  <div id="status"></div>
</div>
<script>
document.addEventListener("DOMContentLoaded", function() {
  ...
  form.addEventListener("submit", function(e) {
    ...
    status.innerHTML = "Checking changes on GitHub...";

    var body = "github_token=" + encodeURIComponent(token);
    ...
    fetch("/push-to-github", {
      ...
      if (j.success) {
        var files = j.changed_files || j.files_uploaded || 0;
        status.className = "ok";
        status.innerHTML = "<b>" + files + "</b> changed files pushed!" +
          "<br>" + (j.message || "") +
          "<br>Commit: <code>" + j.commit_sha + "</code>" +
          '<br><a href="' + j.repo_url + '" target="_blank" style="color:#95d5b2">GitHub &#x2192;</a>';
      } else {
        status.className = "err";
        status.innerHTML = "Error: " + (j.error || "Unknown error");
      }
    }).catch(function(err) {
      status.className = "err";
    ...
    skip_dirs = {'.git', '__pycache__', '.hermes', 'node_modules', '.vercel', 'venv', '.venv', '.cache', 'instance', 'static/gen', 'lang', '.config', 'tmp', 'tmpi*'}
    skip_ext = ('.pyc', '.pyo', '.db', '.zip', '.bundle')

    # Step 1: Get current branch ref + full tree (single API call each)
    ...
    # Get existing tree: path -> {sha, size}
    tree_resp = _gh('GET', f'/repos/{owner}/{repo_name}/git/trees/{base_sha}?recursive=1')
    remote = {}  # path -> {sha, size}
    for item in tree_resp.get('tree', []):
        if item['type'] == 'blob':
            remote[item['path']] = {'sha': item['sha'], 'size': item.get('size', 0)}

    # Step 2: Scan local files
    local_files = {}  # path -> raw_bytes
    ...
    if not local_files:
        return jsonify(success=False, error='No local files found'), 400

    # Step 3: Find changed files (by size comparison — fast)
    changed = [rp for rp in local_files if rp not in remote or len(local_files[rp]) != remote[rp]['size']]

    if not changed:
        return jsonify(success=True, changed_files=0, commit_sha='none', message='No changes detected! ✅', repo_url=f'https://github.com/{owner}/{repo_name}')

    # Step 4: Create blobs ONLY for changed files (parallel)
    def _mkblob(item):
        rp, raw = item
        b64c = _b64.b64encode(raw).decode()
        return rp, _gh('POST', f'/repos/{owner}/{repo_name}/git/blobs', {'content': b64c, 'encoding': 'base64'})

    new_blobs = {}
    from concurrent.futures import ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=8) as pool:
        for rp, res in pool.map(_mkblob, [(rp, local_files[rp]) for rp in changed]):
            if 'sha' in res:
                new_blobs[rp] = res['sha']

    if not new_blobs:
        return jsonify(success=False, error='Blob creation failed'), 500

    # Step 5: Build tree — ONLY changed files (base_tree keeps the rest)
    tree_items = []
    for rp in changed:
        if rp in new_blobs:
            tree_items.append({'path': rp, 'mode': '100644', 'type': 'blob', 'sha': new_blobs[rp]})

    # Step 6: Create tree with base_tree (GitHub merges with existing)
    ...
    changed_names = ', '.join([p.split('/')[-1] for p in changed[:5]])
    commit_msg = f'Security fix ({len(changed)} files): {changed_names}'

    commit = _gh('POST', f'/repos/{owner}/{repo_name}/git/commits', {'message': commit_msg, 'tree': tree['sha'], 'parents': [base_sha]})
    ...
    return jsonify(success=True, changed_files=len(changed), commit_sha=commit['sha'][:7], message=f'Changed: {changed_names}', repo_url=f'https://github.com/{owner}/{repo_name}')

@app.route('/cleanup-github', methods=['GET', 'POST'])
def cleanup_github():
    """Remove Vercel build artifacts from GitHub repo"""
    import urllib.request as _urlreq
    import urllib.error as _urlerr
...
input{width:100%;padding:14px;background:#16213e;border:1px solid #333;border-radius:8px;color:#fff;font-size:15px;margin-bottom:14px;font-family:monospace}
input:focus{outline:none;border-color:#4CAF50}
.btn{width:100%;padding:14px;background:#e74c3c;border:none;border-radius:8px;color:#fff;font-size:15px;font-weight:700;cursor:pointer}
...
<body>
<div class="card">
  <h1>🧹 Cleanup GitHub</h1>
  <p class="sub">Remove lang/, lib/, log/, runtime/, task/ from repo</p>
  <form id="cleanupForm">
    <input type="password" id="tok" placeholder="ghp_xx...xxxx" required autocomplete="off">
    <button class="btn" type="submit" id="btn">🧹 Cleanup</button>
  </form>
  <div id="status"></div>
</div>
<script>
document.addEventListener("DOMContentLoaded", function() {
  ...
  form.addEventListener("submit", function(e) {
    ...
    status.className = "wait";
    status.style.display = "block";
    status.innerHTML = "Scanning repo for artifacts...";
    fetch("/cleanup-github", {
      method: "POST",
      headers: {"Content-Type": "application/x-www-form-urlencoded"},
      body: "github_token=" + encodeURIComponent(token)
    }).then(function(r) { return r.json(); })
    .then(function(j) {
      if (j.success) {
        status.className = "ok";
        status.innerHTML = "Removed <b>" + j.removed + "</b> artifact files!" +
          "<br>" + (j.message || "") +
          "<br>Commit: <code>" + j.commit_sha + "</code>" +
          '<br><a href="' + j.repo_url + '" target="_blank" style="color:#95d5b2">GitHub &#x2192;</a>';
      } else {
        status.className = "err";
        status.innerHTML = "Error: " + (j.error || "Unknown");
      }
    }).catch(function(err) {
      status.className = "err";
    ...
    # Artifact directories to remove
    artifact_dirs = {'lang', 'lib', 'log', 'runtime', 'task'}
    ...
    if 'tree' not in tree_resp:
        return jsonify(success=False, error=f'Tree error: {tree_resp.get("error")}')

    # Find artifact files to keep
    keep = []
    removed_count = 0
    for item in tree_resp['tree']:
        if item['type'] != 'blob':
            continue
        top_dir = item['path'].split('/')[0]
        if top_dir in artifact_dirs:
            removed_count += 1
        else:
            keep.append({'path': item['path'], 'mode': '100644', 'type': 'blob', 'sha': item['sha']})

    if removed_count == 0:
        return jsonify(success=True, removed=0, commit_sha='none', message='No artifacts found!', repo_url=f'https://github.com/{owner}/{repo_name}')

    # Create new tree without artifacts
    tree = _gh('POST', f'/repos/{owner}/{repo_name}/git/trees', {'tree': keep})
    if 'sha' not in tree:
        return jsonify(success=False, error=f'Tree failed: {tree.get("error")}')

    commit = _gh('POST', f'/repos/{owner}/{repo_name}/git/commits', {
        'message': f'🧹 Cleanup: remove {removed_count} Vercel build artifacts ({", ".join(sorted(artifact_dirs))})',
        'tree': tree['sha'],
        'parents': [base_sha]
    })
    if 'sha' not in commit:
    ...
    return jsonify(success=True, removed=removed_count, commit_sha=commit['sha'][:7], message=f'Removed: {", ".join(sorted(artifact_dirs))}', repo_url=f'https://github.com/{owner}/{repo_name}')

@app.route('/debug/whatsapp')
def debug_whatsapp():
    """Debug: Check WhatsApp config (admin only)"""
    # MEDIUM #13: Disable in production
    if not app.debug:
...
def whatsapp_webhook():
    ...
    if request.method == 'GET':
        ...
        # CRITICAL #3: Verify WhatsApp webhook signature
        ...
        if app_secret:
            ...
            # No app_secret configured — reject unsigned requests
            signature = request.headers.get('X-Hub-Signature-256', '')
            if not signature:
                return 'Webhook not configured (no WHATSAPP_APP_SECRET)', 403
        ...
        try:
            ...
            if 'image' in request.files:
                ...
                if file and file.filename:
                    # CRITICAL #7: Validate upload
                    valid, msg = validate_upload(file)
                    if not valid:
                        flash(f'Upload gagal: {msg}', 'error')
...
def product_edit(product_id):
    ...
    # IDOR #12: Validate product belongs to this shop
    if product.shop_id and product.shop_id != shop.id:
        flash('Produk ini bukan milik toko Anda!', 'danger')
        return redirect(url_for('products_page'))
...
def demo_test():
    ...
    try:
        ...
        result = {
            'response': 'Maaf, AI sedang sibuk. Coba lagi beberapa saat. / Sorry, AI is busy. Try again later.',
            'intent': 'error',
            'response_time_ms': 0
        }
